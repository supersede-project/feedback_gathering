...
public interface UserGroupRepository extends JpaRepository<UserGroup, Long> {
    ...
    @Query(value = "SELECT g.id, g.name, g.application_id " +
            ...
            "    WHERE g.id = u.user_group_id and g.application_id = ?1 and u.user_identification = ?2 LIMIT 1", nativeQuery = true)
    UserGroup findByApplicationIdAndUserIdentification(long applicationId, String userIdentification);
}
